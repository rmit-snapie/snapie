...
class TakePhoto extends Component {
    ...
    if (imageUri) {
      return (
        <View style={styles.backgroundContainer}>
          <ImageBackground style={styles.previewImg} source={{uri: imageUri}} />
            ...
            {this.state.result.map((item, key) => (
              <View
                ...
                <View style={{flexDirection: 'row'}}>
                  <Button onPress={() => readText(item.label)} style={{fontSize: 20}} title="listen" />
  ...
  analyze() {
    ...
    axios
      .post(
        'https://asia-northeast1-rmit-sepm.cloudfunctions.net/getImageLabels',
        {
      ...
      .then(response => {
        for (let i = 0; i < response.data.length; i++) {
          axios
            .post(
              'https://asia-northeast1-rmit-sepm.cloudfunctions.net/searchImageByKeyword',
              {
